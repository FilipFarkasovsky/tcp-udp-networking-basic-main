...
public class SimpleInterpolation : MonoBehaviour
{
    ...
    private void Start()
    {
        to = new TransformUpdate(GlobalVariables.clientTick, transform, isLocalRotation);
        from = new TransformUpdate(GlobalVariables.clientTick- Utils.timeToTicks(0.1f), transform, isLocalRotation);
        previous = new TransformUpdate(GlobalVariables.clientTick - Utils.timeToTicks(0.1f), transform, isLocalRotation);
    }
    ...
    {
        if (_tick <= GlobalVariables.clientTick - Utils.timeToTicks(0.1f))
        {
            return;
        }
    ...
    internal class TransformUpdate
    {
        ...
        internal TransformUpdate(int _tick, Transform _transform, bool isLocalRotation = false)
        {
            tick = _tick;
